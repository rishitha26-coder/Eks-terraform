...
ls -la /vault/userconfig/vault-server-tls/*
kubectl exec -t vault-1 -- "/bin/sh" "-c" "vault operator raft join -tls-skip-verify -leader-ca-cert=\"$(cat /vault/userconfig/vault-server-tls/ca.crt)\" ${PROTOCOL}://${VAULT_0}:${VAULT_PORT}"
kubectl exec -t vault-2 -- "/bin/sh" "-c" "vault operator raft join -tls-skip-verify -leader-ca-cert=\"$(cat /vault/userconfig/vault-server-tls/ca.crt)\" ${PROTOCOL}://${VAULT_0}:${VAULT_PORT}"
